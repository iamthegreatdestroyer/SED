...
async function syncDependencies(options: SyncOptions = {}): Promise<void> {
  ...
  for (const update of updates) {
    console.log(
      `  ${update.pkg}: ${update.dep} ${chalk.red(update.from)} → ${chalk.green(update.to)}`
    );
  }
...
async function showVersions(): Promise<void> {
  ...
  for (const pkgPath of packagePaths.filter((p) => p.includes('/packages/'))) {
    const pkg = await fs.readJson(pkgPath);
    const indicator = pkg.version === rootPkg.version ? chalk.green('✓') : chalk.red('✗');
...
program.command('show').description('Show all package versions').action(showVersions);
