...
async function buildPackages(options: BuildOptions = {}): Promise<void> {
  ...
  try {
    const filter = options.filter ? `--filter=${options.filter}` : '--filter="./packages/*"';
...
async function buildApps(options: BuildOptions = {}): Promise<void> {
  ...
  try {
    const filter = options.filter ? `--filter=${options.filter}` : '--filter="./apps/*"';
...
async function clean(dirs: string[] = ['packages', 'apps', 'docs']): Promise<void> {
  ...
  const cleanPaths = ['dist', '.turbo', 'coverage', '*.tsbuildinfo'];
...
// CLI
program.name('build').description('SED build utilities').version('0.1.0');
...
program.command('docs').description('Build documentation').action(buildDocs);

program.command('prod').description('Full production build with tests').action(buildForProduction);
