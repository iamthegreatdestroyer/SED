 ...
 */
export function formatEntropyScore(analysis: EntropyAnalysis, colorize = true): string {
 ...
 */
export function formatEntropyBar(value: number, width = 20, colorize = true): string {
...
export function box(text: string, title?: string): string {
  ...
  const boxedLines = lines.map((line) => `│ ${line.padEnd(maxWidth)} │`);
