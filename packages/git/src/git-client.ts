...
export class GitClient {
  ...
  async getChangedFiles(fromRef: string, toRef: string = 'HEAD'): Promise<FileChange[]> {
    const diff = await this.git.diff(['--name-status', '--numstat', fromRef, toRef]);
   ...
   */
  async getFileDiff(filePath: string, fromRef: string, toRef: string = 'HEAD'): Promise<string> {
  ...
  async getLog(
    ...
    const logOptions: string[] = ['--format=%H|%h|%an|%ae|%aI|%s|%b|%D'];
