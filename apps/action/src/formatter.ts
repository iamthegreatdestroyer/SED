...
export function formatMarkdownReport(analysis: AnalysisResult): string {
  ...
  lines.push('');
  lines.push(`**Commit Range:** \`${analysis.from}\` → \`${analysis.to}\``);
  ...
  lines.push('---');
  lines.push(
    '*Generated by [SED - Semantic Entropy Differencing](https://github.com/sgbilod/sed)*'
  );
...
export function formatPRComment(analysis: AnalysisResult): string {
  ...
  lines.push('## 🔍 SED Analysis Results');
  lines.push('');
  ...
  if (hasCritical || hasHigh) {
    ...
    if (hasCritical) {
      lines.push(
        `- 🔴 **${analysis.summary.classifications.critical}** files with critical entropy`
      );
    }
    if (hasHigh) {
      lines.push(`- 🟠 **${analysis.summary.classifications.high}** files with high entropy`);
    }
    lines.push('');
  }
  ...
  if (analysis.files.length > 0) {
    lines.push('<details>');
    lines.push('<summary>📁 Top Entropy Files (click to expand)</summary>');
    lines.push('');
    lines.push('| File | Entropy | Classification |');
    lines.push('|------|---------|----------------|');
  ...
  lines.push('---');
  lines.push('<sub>🤖 Generated by SED - Semantic Entropy Differencing</sub>');
