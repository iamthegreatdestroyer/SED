...
export async function runAction(
  ...
  const include = inputs.include
    ? inputs.include.split(',').map((p) => p.trim())
    : undefined;
  ...
  const { shouldFail, failureMessage } = checkFailureConditions(
    inputs,
    analysis,
    classification
  );
...
async function writeSummary(
  ...
  await core.summary
    ...
    .addTable([
      ...
      [
        'Classification',
        `${classificationEmoji[classification]} ${classification.toUpperCase()}`,
      ],
    ])
    .addBreak()
    .addDetails(
...
async function postPRComment(
  ...
  const existingComment = comments.find(
    (c) => c.body?.includes('<!-- SED Analysis -->')
  );
