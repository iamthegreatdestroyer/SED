...
export function generateMarkdownReport(
  ...
  md += `| Status | ${data.repository.isClean ? '✅ Clean' : '⚠️ Modified'} |\n\n`;
  ...
  for (const file of data.files) {
    ...
    if (file.error) {
      md += `| ${statusEmoji} | \`${file.file}\` | ${file.language || '-'} | ❌ Error | Failed |\n`;
    } else {
  ...
  if (data.commits.length > 0) {
    ...
    for (const commit of data.commits.slice(0, 50)) {
      ...
      const conventional = commit.isConventional ? '✅' : '';
