...
describe('EntropyAnalyzer', () => {
  ...
  describe('getRecommendations', () => {
    ...
    it('should recommend review for high entropy', () => {
      const result = analyzer.analyze([
        createNodeEntropy('a', 'class', 0.8, 'high'),
      ]);
    ...
    it('should provide specific recommendations for critical entropy', () => {
      const result = analyzer.analyze([
        createNodeEntropy('a', 'module', 0.95, 'critical'),
      ]);
      ...
      expect(
        recommendations.some(
          (r) =>
            ...
            r.toLowerCase().includes('smaller')
        )
      ).toBe(true);
    });
  });
  ...
  describe('compareAnalyses', () => {
    it('should compare two analyses', () => {
      const before = analyzer.analyze([
        createNodeEntropy('a', 'function', 0.3, 'low'),
      ]);
      const after = analyzer.analyze([
        createNodeEntropy('a', 'function', 0.5, 'moderate'),
      ]);
    ...
    it('should detect improvement', () => {
      const before = analyzer.analyze([
        createNodeEntropy('a', 'function', 0.8, 'high'),
      ]);
      const after = analyzer.analyze([
        createNodeEntropy('a', 'function', 0.3, 'low'),
      ]);
    ...
    it('should detect regression', () => {
      ...
      const after = analyzer.analyze([
        createNodeEntropy('a', 'function', 0.9, 'critical'),
      ]);
