...
export class EntropyAnalyzer {
  ...
  getRecommendations(analysis: EntropyAnalysis): string[] {
    ...
    if (addedCount > totalChanges * 0.7) {
      recommendations.push(
        '➕ Mostly additions. Ensure new code follows existing patterns.'
      );
    }

    if (removedCount > totalChanges * 0.5) {
      recommendations.push(
        '➖ Significant removals. Verify no unintended functionality is lost.'
      );
    }
  ...
  private calculateDistribution(
    nodeEntropies: NodeEntropy[]
  ): Record<EntropyLevel, number> {
