...
const BUILT_IN_RULES: ClassificationRule[] = [
  ...
  },
  {
    ...
    condition: (change) => change.nodeType === 'class' && change.changeType === 'modified',
    tag: 'class-change',
    riskMultiplier: 1.2,
  ...
  },
  {
    ...
    condition: (change) => change.modifications?.some((m) => m.type === 'structure') ?? false,
    tag: 'structural',
    riskMultiplier: 1.25,
...
export class ChangeClassifier {
   ...
   */
  classify(change: SemanticChange, entropy: NodeEntropy): ChangeClassification {
    // Find matching rules
    const matchingRules = this.rules.filter((rule) => rule.condition(change, entropy));
   ...
   */
  getReviewRequired(classifications: ChangeClassification[]): ChangeClassification[] {
   ...
   */
  getByLevel(classifications: ChangeClassification[], level: EntropyLevel): ChangeClassification[] {
   ...
   */
  getByTag(classifications: ChangeClassification[], tag: string): ChangeClassification[] {
  ...
  generateSummary(classifications: ChangeClassification[]): {
    ...
    const averageRiskScore =
      classifications.length > 0
        ? classifications.reduce((sum, c) => sum + c.riskScore, 0) / classifications.length
        : 0;

    // Generate recommendations
    const recommendations = this.generateRecommendations(classifications, byLevel, topTags);
