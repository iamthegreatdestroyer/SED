...
export class SEDEngine {
  ...
  async compare(input: CompareInput): Promise<SEDResult> {
    ...
    const entropyMap = new Map(
      analysis.nodeEntropies.map((e) => [e.nodeId, e])
    );
    const classifications = this.changeClassifier.classifyBatch(
      diff.changes,
      entropyMap
    );

    // Generate summary
    const classificationSummary = this.changeClassifier.generateSummary(
      classifications
    );
