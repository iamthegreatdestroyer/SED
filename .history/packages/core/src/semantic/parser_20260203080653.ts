...
export class SemanticParser {
  ...
  private buildHierarchy(nodes: SemanticNode[]): SemanticNode[] {
    ...
    for (const node of sorted) {
      ...
      while (
        stack.length > 0 &&
        !this.nodeContains(stack[stack.length - 1]!, node)
      ) {
        stack.pop();
      }
  ...
  private extractErrors(rootNode: { children: unknown[]; type: string }): string[] {
    ...
    const walk = (node: { children?: unknown[]; type: string; isMissing?: boolean; startPosition?: { row: number; column: number } }): void => {
