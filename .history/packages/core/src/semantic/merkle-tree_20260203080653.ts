...
export class MerkleTreeBuilder {
  ...
  build(nodes: SemanticNode[]): { roots: MerkleNode[]; stats: MerkleTreeStats } {
    ...
    const buildMerkleNode = (node: SemanticNode, depth: number): MerkleNode => {
      ...
      const merkleChildren = node.children.map((child) =>
        buildMerkleNode(child, depth + 1)
      );
  ...
  findChangedSubtrees(
    oldRoots: MerkleNode[],
    newRoots: MerkleNode[]
  ): { path: string[]; oldNode?: MerkleNode; newNode?: MerkleNode; changeType: 'added' | 'removed' | 'modified' }[] {
    const changes: { path: string[]; oldNode?: MerkleNode; newNode?: MerkleNode; changeType: 'added' | 'removed' | 'modified' }[] = [];
